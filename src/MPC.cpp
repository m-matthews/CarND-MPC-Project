...
size_t N = 10;     // 8 doesn't hold to polynomial, 16 is jittery, 12 better than 8 but still a bit jittery.  10 selected.
double dt = 0.100; // Initial selection to match the latency so predictions will match measurement interval.

// The solver takes all the state variables and actuator variables in a singular vector.
// The following is used to determine where one variable starts and another ends.
size_t x_start = 0;
...
size_t a_start = delta_start + N - 1;  // '- 1' as the 'delta' and 'a' vectors are one element shorter.
 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (unsigned int t = 0; t < N - 1; ++t) {
      fg[0] +=  100 * CppAD::pow(vars[delta_start + t], 2); // Milder steering.
      fg[0] +=   10 * CppAD::pow(vars[a_start + t], 2);     // Lower acceleration.
    }

    // Minimize the value gap between sequential actuations.
    for (unsigned int t = 0; t < N - 2; ++t) {
      fg[0] +=  100 * CppAD::pow(vars[delta_start + t + 1] - vars[delta_start + t], 2); // Minimise abrupt steering.
      fg[0] +=    1 * CppAD::pow(vars[a_start + t + 1] - vars[a_start + t], 2);         // Minimise abrupt acceleration.
    }
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs, bool latency) {
  ...
  for (unsigned int i = delta_start; i < n_vars; ++i) {
    vars_lowerbound[i] = -1.0;  // Main.cpp comment for steering and throttle: "Both are in between [-1, 1].".
